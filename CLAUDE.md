...
### كلفة الجهاز — الطبيب يحدّدها (migration 030)
`medical_exams.device_cost` — **للأطراف والمساند فقط**، لأن الطبيب هو مَن
يحدّد الجهاز فيعرف كلفته. تُكتب على الحالة بـ `cost_source = 'manual'`
فلا تلمسها الأتمتة. ويبقى للاستعلامات **القبض وإسناد الخبير**.
