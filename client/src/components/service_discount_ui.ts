...
/**
 * **هل «المبلغ المدفوع الآن» إلزاميٌّ؟** — تصحيحٌ تشغيليّ (الجهوزيةُ
 * الماليةُ، القسم الثاني).
 *
 * ══ العطبُ الذي تغلقه ════════════════════════════════════════════════
 * كان الشرطُ في الشاشة `!hasDiscount(d, originalPrice)` — و`hasDiscount`
 * تُرجع `true` لكلّ خصم، **مجّانياً كان أو مخفَّضاً فقط**. فخصمٌ حقيقيّ
 * (١٠٠,٠٠٠ ⟵ ٨٠,٠٠٠، ليس تبرّعاً) كان يُعفى من الإلزام بالخطأ — والمريضُ
 * لا يزال يدفع مبلغاً حقيقياً موجباً له، تماماً كالسعر الكامل.
 *
 * **والاستثناءُ الوحيد تبرّعٌ صريح** (`isFree`) — الخادمُ يفرض عليه الصفرَ
 * بصرف النظر عمّا وصل، فلا معنى لإلزامه مبلغاً. **وليس مضاعفَ سعر الجلسة**:
 * أيّ مبلغٍ موجبٍ يكفي، جزئياً كان أو كاملاً — هذه الدالّةُ تقول «أإلزامٌ
 * أم لا؟» فقط، والمقارنةُ بصفرٍ عند المستدعي.
 */
export function paymentEntryRequired(d: DiscountDraft, serviceCost: number): boolean {
  return d.isFree !== true && serviceCost > 0;
}
