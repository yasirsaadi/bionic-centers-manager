...
//  المربّعُ يُفكَّك حين تُفتَح لوحةُ التدريب (أو «محادثاتي») ويُعاد تركيبُه
//  حين تُغلَق، و`draft`/`open` لا يتغيّران عبر ذلك التبديل — فمسوّدةُ سطرين
//  كانت تبقى في مربّعٍ ارتفاعُه سطرٌ واحد حتى يكتب الموظّفُ حرفاً آخر.
//
//  **والمرساةُ لا تُثبَّت على شرطٍ بعينه**: الشرطُ اكتسب `!historyOpen` في
//  ٢٠٢٦-٠٩-٢١، وقد يكتسب ثالثاً. والمقصودُ أن يكون التفكيكُ **واقعاً** —
//  وذاك ما تقوله هـ.١٠ج نفسُها: أيُّ إعادة تركيبٍ تُقاس مهما كان الشرطُ
//  الذي فكّكه. فتُطابَق بنيةُ الحارس لا نصُّ شروطه، وتسقط إن أُزيل الحارسُ.
const guard = /\{(?:\s*![A-Za-z][A-Za-z0-9_]*\s*&&)+\s*\(\s*<form/;
const guardAt = drawer.slice(0, i).search(guard) >= 0
  ? drawer.slice(0, i).lastIndexOf("{!") : -1;
check(guard.test(drawer) && guardAt > 0 && guardAt < i,
  "هـ.١٠ **والمربّعُ مشروطٌ فعلاً بإغلاق اللوحات** — فالتفكيكُ واقعٌ لا " +
