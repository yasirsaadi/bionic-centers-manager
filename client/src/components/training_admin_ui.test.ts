...
console.log("\n── القسمُ ٣ — لوحةُ إدارة المسارات والوحدات ──");
{
  ...
  //  مراجعةٌ حيّة ٢٠٢٦-٠٩-١١ (تصحيحٌ ثالث، القسم ٣) — قائمةُ اختيار المقالات
  //  تستبعد الخاصّةَ بفرعٍ **قبل** الإرسال، لا تنتظر ردَّ ٤٠٠ من الخادم فقط.
  check(/\.filter\(\(a\)\s*=>\s*a\.isActive\s*&&\s*a\.branchId\s*===\s*null\)/.test(tabBody),
    "هـ.٥ **وقائمةُ المقالات تستبعد الخاصّةَ بفرعٍ صراحةً (`branchId === null`)** — لا يختار المسؤولُ مرجعاً سيرفضه الخادمُ لاحقاً");
  ...
  //  الشرطُ صار مركَّباً (فعّالةٌ **و**عامّة) منذ تصحيحٍ ثالثٍ حيّ — راجع هـ.٥
  //  للفحص الدقيق لشطر `branchId === null`؛ هنا يكفي إثباتُ بقاء شطر
  //  الفعّالة (`a.isActive`) ضمن `.filter(` نفسِها بلا مسّ.
  check(/\.filter\(\(a\)\s*=>\s*a\.isActive/.test(tabBody),
