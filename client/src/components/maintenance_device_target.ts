 ...
 * لا ينتهي. `isSuccess` وحدها تحسم `choose`/`none`؛ وكلُّ ما دونهما (تحميلٌ،
 * إعادةُ جلب، أو استعلامٌ لم يُفعَّل بعد لأن المريضَ أو الخدمةَ لم يُعرَفا
 * بعد) يبقى `loading` — **آمنٌ دائماً**: يمنع الحفظَ ولا يُخترَع منه غياب.
 */
export function devicePhaseOf(q: DeviceQueryState): MaintenanceDevicePhase {
  if (q.isError) return "error";
  if (q.isSuccess) return q.hasOptions ? "choose" : "none";
  return "loading";
}
