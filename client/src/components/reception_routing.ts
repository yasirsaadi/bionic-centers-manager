...
// طبية»، الذي يفتح نافذةَ «جهاز جديد» القائمة نفسَها **بمسار «نعم» معبَّأً
// مسبقاً** (`initialServicePath`) — لا مقفولاً: هذا توجيهٌ تشغيليّ لا قرارٌ
// سريريّ موقَّع، فيبقى محدِّدُ «هل تحتاج معاينة؟» داخل النافذة مفتوحاً
// كأيّ محدِّدٍ آخر، ويصحّحه الاستقبالُ إلى «بلا معاينة» متى تبيّن أن
// المريض لا يحتاج طبيباً فعلاً — بلا تدخّل إداريّ. **والموظّفُ يحدّد
 ...
 *
 * و«يحتاج معاينة طبية» **يُعبِّئ** سؤالَ المعاينة داخل نافذة «جهاز جديد»
 * بـ`"exam"` (`initialServicePath`) — اقتراحٌ لا قفل: الموظّفُ يستطيع
 * تبديله إلى «بلا معاينة» من داخل النافذة نفسِها إن تبيّن ذلك لاحقاً،
 * فلا تناقضَ بين اسم الزرّ ومحدِّدٍ يبقى مفتوحاً داخل ما فتحه.
 *
  ...
  return [
    {
      ...
      flow: { kind: "device_episode", serviceType, initialServicePath: "exam" },
    },
    {
      id: "device_sale",
