...
// طبية»، الذي يفتح نافذةَ «جهاز جديد» القائمة نفسَها. **والموظّفُ يحدّد
// المطلوبَ فيها كما كان دائماً** (طرفٌ كاملٌ أو جزء) — لكن **مسارَ
// المعاينة محسومٌ ومقفولٌ على «نعم»**: اختيارُ هذا الزرّ بعينه أجاب عن
// سؤال «هل تحتاج معاينة؟»، فلا يُعاد طرحُه ولا يُسمَح بتبديله إلى «بلا
// معاينة» من نافذةٍ فُتحت أصلاً لأنها تحتاج معاينة. التناقضُ الذي كان:
// نافذةٌ تُفتَح باسم «يحتاج معاينة طبية» ثم تسأل «هل تحتاج معاينة؟».
//
 ...
 *
 * و«يحتاج معاينة طبية» يقفل سؤالَ المعاينة داخل نافذة «جهاز جديد» على
 * `"exam"` (`lockedServicePath`) — فلا تناقضَ بين اسم الزرّ وسؤالٍ يُعاد
 * طرحُه داخل ما فتحه.
 *
  ...
  return [
    {
      ...
      flow: { kind: "device_episode", serviceType, lockedServicePath: "exam" },
    },
    {
      id: "device_sale",
