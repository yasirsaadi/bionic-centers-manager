...
export default function EditPatient() {
  ...
  useEffect(() => {
    if (patient) {
      const parseCommaSeparated = (val: string | null | undefined): string[] => {
        if (!val) return [];
        return val.split(/، |, /).map(s => s.trim()).filter(Boolean);
      };
      setSelectedInjuryTypes(parseCommaSeparated(patient.injuryType));
      setSelectedInjuryAreas(parseCommaSeparated(patient.injuryArea));
      form.setValue("injuryType", patient.injuryType || "");
      form.setValue("injuryArea", patient.injuryArea || "");
    }
  }, [patient]);
