...
// ══ د٢. البادئة التدريجية — والاسم لا يتسلّل إليها ══════════════════════
console.log("\n── بادئةُ الرمز ──");
const PFX: Row[] = [
  { id: "c-02119", patientName: "زيدان فالح", patientCode: "WB-02119",
    waitingSince: "2026-08-03T00:00:00Z" },
  { id: "c-02110", patientName: "سجاد نوري", patientCode: "WB-02110",
    waitingSince: "2026-08-02T00:00:00Z" },
  { id: "c-09090", patientName: "كرار حسن", patientCode: "WB-09090",
    aliasCodes: ["WB-02900"], waitingSince: "2026-08-01T00:00:00Z" },
  //  **الفخّ**: اسمُه نفسه يبدأ بـ«WB-02» ورمزُه بعيدٌ عنها. فلو تسرّب بحثُ
  //  الاسم إلى عالم الرموز لظهر هذا فوق مَن يحمل البادئة في رمزه فعلاً.
  { id: "n-fake", patientName: "WB-02 مركز الأطراف", patientCode: "WB-71234",
    waitingSince: "2026-08-09T00:00:00Z" },
];
same("د٢. «WB» تعرض الجميع (كلُّ رمزٍ يبدأ بها)",
  run(PFX, "WB", "newest").length, 4);
//  والترتيب داخل رتبة البادئة هو ترتيبُ المصدر — أي الانتظار الذي اختاره
//  الطبيب، كبقيّة هذه الصفحة. (وفي سجلّ المرضى المرقَّم يكسر التعادلَ الرمزُ
//  صعوداً بدلاً منه: هناك لا انتظارَ يُعرض، وهنا هو عمودُ الصفحة.)
same("   و«WB-0» تُخرج WB-71234",
  run(PFX, "WB-0", "newest"), ["c-02119", "c-02110", "c-09090"]);
same("   و«WB-02» كذلك", run(PFX, "WB-02", "newest"),
  ["c-02119", "c-02110", "c-09090"]);
//  والاتجاه ينقلب حاملي البادئة في رمزهم الحالي — و«c-09090» يبقى آخراً
//  في الحالتين لأنه دخل ببادئة **اسمه البديل**، ورتبتُها دون رتبة الحالي.
same("   والاتجاه ينقلب حاملي الرمز الحالي",
  run(PFX, "WB-02", "oldest"), ["c-02110", "c-02119", "c-09090"]);
same("   و«WB-021» تضيّق", run(PFX, "WB-021", "newest"), ["c-02119", "c-02110"]);
same("   و«wb02» بلا شَرطة", run(PFX, "wb02", "newest"),
  ["c-02119", "c-02110", "c-09090"]);
same("   و«WB-٠٢١» بالأرقام العربية", run(PFX, "WB-٠٢١", "newest"),
  ["c-02119", "c-02110"]);
same("   والرمز الكامل يتصدّر", run(PFX, "WB-02119", "newest")[0], "c-02119");
same("   والاسم البديل الكامل يتصدّر كذلك",
  run(PFX, "WB-02900", "newest")[0], "c-09090");
same("   وبادئةُ الاسم البديل تُدخل صاحبها",
  run(PFX, "WB-029", "newest"), ["c-09090"]);
//  الحارس: بدأ المكتوب بـWB ⟶ لا اسمَ ولا هاتفَ ولا تقريب.
check(!run(PFX, "WB-02", "newest").includes("n-fake"),
  "   **واسمٌ يبدأ نصُّه بـ«WB-02» لا يظهر لأن رمزه لا يبدأ بها**",
  JSON.stringify(run(PFX, "WB-02", "newest")));
check(!run(PFX, "WB-021", "newest").includes("n-fake"),
  "   ولا في بادئةٍ أطول");
//  وهو يُوجَد باسمه كالمعتاد حين لا يكون البحث بادئةَ رمز.
same("   ويُوجَد باسمه كالمعتاد", run(PFX, "مركز", "newest"), ["n-fake"]);
same("   ولا تسامحَ مع خطأٍ في الرمز", run(PFX, "WB-02118", "newest"), []);
