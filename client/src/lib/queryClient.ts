...
/**
 * نقاطٌ تُنادى بـ`POST` **وهي لا تغيّر بياناتِ عملٍ تُعرَض في شاشة**.
 *
 * وأخطرُ ما فيها ما يُستعمَل `queryFn` لاستعلامٍ **فعّال**: التحديثُ يُعيد
 * جلبَه، وإعادةُ الجلب تنادي النقطةَ ثانيةً فتُجدول تحديثاً — **حلقةٌ لا
 * تنتهي كلَّ ٥٠ مللي ثانية** ما دامت الشاشةُ مفتوحة. مُعادةٌ حيّاً قبل هذا
 * الإصلاح: ٢٤ طلباً في ١٢٠٠ مللي ثانية لبطاقة تلميحات المصروف، و٢٠ طلباً
 * في ١٠٠٠ لنافذة التصحيح الإداريّ.
 *
 * **ولا يُضاف هنا إلّا ما لا يكتب حرفاً في القاعدة** — يُفحَص في الخادم
 * أوّلاً، **ويحرسه فحصٌ معماريّ** في `test:live-refresh` يقرأ مصادر الواجهة
 * فيُفشل الحزمةَ حين تظهر نقطةُ كتابةٍ جديدة داخل `queryFn` لم يُقرَّر فيها.
 */
const LIVE_REFRESH_EXEMPT = [
  //  رسالةٌ للمساعد ليست تغييرَ بيانات.
  /^\/api\/ai\/chat$/,
  //  تلميحاتُ نموذج المصروف — قواعدُ تُحسَب وتُرَدّ، بلا كتابة (`routes.ts`).
  /^\/api\/guidance\/expense$/,
  //  أثرُ التصحيح قبل وقوعه — `previewReversal` قراءةٌ محضة، والكتابةُ في
  //  `execute` وحدها (§4.f: «النافذةُ وحدها تنادي `preview` و`execute`»).
...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      ...
      //
      //  **و«always» لا `true`**: الثانيةُ تُعيد جلبَ **القديم وحده**، و
      //  `staleTime` أدناه ستّون ثانية — فزميلٌ يكتب بينما تبويبُك مخفيّ ثمّ
      //  تعود خلال دقيقة **لا يقع طلبٌ إطلاقاً**، ولا حدثَ تركيزٍ ثانياً
