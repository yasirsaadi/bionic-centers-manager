...
async function main() {
  ...
  try {
    ...
    console.log("\n── س. عقدُ الشاشة ──");
    {
      ...
      //  ⚠ **انقلب عقدُ البندين ٧ و٨ بقرار المالك (٢٠٢٦-٠٩-٢٤، شكوى «زهراء»)** —
      //  كانت القائمةُ تُطلب **بفرع المتابعة**، فاستقبالُ فرعٍ أُتيح له الملفّ
      //  يُردّ ٤٠٣ فيرى قائمةً فارغة بصمت، والمسؤولُ لا يرى خبيرَ الفرع
      //  المضاف (أيوب). فصارت تُطلب **بالمريض** — فروعُ ملفّه ∩ نطاقُ الفاعل،
      //  القاعدةُ نفسُها التي يحسم بها الخادمُ فرعَ البيع عند الحفظ. **والمقصدُ
      //  الأصليّ باقٍ**: المفتاحُ يحمل هويّةً فلا تُعرَض قائمةٌ على غير صاحبها،
      //  والطلبُ يحمل الهويّةَ نفسَها.
      check(
        /queryKey:\s*saleExpertsQueryKey\(patientId\)/.test(shared)
        && !/queryKey:\s*\["\/api\/manufacturing\/experts",\s*branchId\]/.test(shared),
        "٧. **ومفتاحُ استعلام المكوّن المشترك يحمل هويّةَ المريض** (`saleExpertsQueryKey`)"
        + " — لا مفتاحٌ عامّ ولا مفتاحُ فرع المتابعة (انقلب بقرار المالك)");
      check(
        /queryFn:\s*\(\)\s*=>\s*fetchSaleExperts\(patientId\)/.test(shared)
        && !/\/api\/manufacturing\/experts\?branchId=/.test(shared),
        "٨. **والطلبُ الفعليّ في المكوّن المشترك بالمريض** (`fetchSaleExperts` ⟵ `?patientId=`)"
        + " — لا `?branchId=<فرع المتابعة>` (انقلب بقرار المالك)");
    ...
    //  ⚠ **ثمّ صارت نافذةُ البيع تطلبها بالمريض** (٢٠٢٦-٠٩-٢٤): فرعُ المتابعة
    //  وحده كان يحجبها عن فرعٍ أُتيح له الملفّ. ووضعُ `?branchId=` باقٍ بحرفه
    //  لبقيّة الشاشات — والبنودُ ١-٥ تحرسه كما هو.
    console.log("\n── ع. قائمةُ الخبراء بفرع العملية ──");
    {
      ...
      //  ٦. **ونافذةُ البيع تطلب القائمةَ بالمريض** (٢٠٢٦-٠٩-٢٤، شكوى «زهراء»)
      //  — والطلبُ بالمريض **لا يوسّعها لكلّ الفروع**: مريضٌ لم يُتَح ملفُّه
      //  لفرعٍ آخر قائمتُه خبراءُ فرعه وحده، **والمسؤولُ العامّ نفسُه لا يرى
      //  خبيرَ فرعٍ لا يصل الملفّ** (`EXPERT_B2` في الفرع ٢ غائب). أمّا المريضُ
      //  المُتاح فقائمتُه تمتدّ على فروعه — مُثبَتٌ حيّاً في
      //  `server/sale_branch_share.test.ts`.
      const { pid: listPid } = await readySale("قائمةُ الخبراء بالمريض");
      for (const [who, sess] of [["الاستقبال", S.recv], ["المحاسب", S.acct],
        ["مديرُ الفرع", S.manager], ["المسؤولُ العام", S.admin]] as any[]) {
        const r = await http("GET", `/api/manufacturing/experts?patientId=${listPid}`, sess);
        same(`٦. **${who}: قائمةُ المريض = خبراءُ فرعه وحده** — لا خبيرَ فرعٍ لا يصل الملفّ`,
          [r.status, namesOf(r.body)], [200, String(EXPERT)]);
      }
    ...
    {
      // ══ معماريّاً: القبضُ الموجب يشترط الهويّةَ الثلاثية كاملةً
      // (تصحيحٌ لاحق) ═══════════════════════════════════════════════════
      //  هذا الفرعُ غيرُ قابلٍ للبناء عبر المسار الحيّ عملياً: `confirmPurchase`
      //  نفسُها تضمن حلقةً (`ensureFirstDeviceEpisodeForSale`) قبل أن يصل
      //  هذا الكودُ، ولا تُرجع فارغاً إلّا حين لا خيطَ للمريض في هذه الخدمة
      //  أصلاً — وهو مستحيلٌ لمتابعةٍ وُلدت عن معاينةٍ موقّعة على حالةٍ
      //  قائمة (نفسُ ما تضمنه `readySale`/`mkCase` دائماً). فتلفيقُ الحالة
      //  بالـSQL الخام كان يعني كسرَ ثابتٍ لا تنتجه أيّ رحلة مستخدمٍ حقيقية
      //  — ففحصٌ معماريٌّ على المصدر نفسِه بدل ذلك، تماماً كنمط القسم ط (١٣).
      const storeSrc = readFileSync(join(import.meta.dirname, "followup", "store.ts"), "utf8");
      const csBody = extractFn(storeSrc, "completeReceptionSale");
      check(
        csBody.length > 0
        && /sold\.caseId === null/.test(csBody)
        && /sold\.branchId === null/.test(csBody)
        && /sold\.deviceEpisodeId === null/.test(csBody),
        "I. **معماريّاً: القبضُ الموجب يشترط الحالةَ والفرعَ وحلقةَ الجهاز الثلاثة معاً"
        + " — لا رصيدَ جهازٍ غيرَ مخصَّص لهذه العملية بعينها**",
        `csBody found=${csBody.length > 0}`);
      check(
        !/البيعُ تمّ؛ سجّل الدفعة/.test(csBody),
        "   **ولا رسالةُ خطأٍ تدّعي أن البيعَ تمّ** — الاستثناءُ يُرمى داخل معاملة البيع"
        + " نفسِها فيتراجع البيعُ معه");
      check(
        /لم تُحفظ العملية/.test(csBody) && /حدّث الملف وحاول مرة أخرى/.test(csBody),
        "   **والرسالةُ الصادقةُ الجديدة موجودة** — تقول إن العمليةَ لم تُحفَظ لا إن البيعَ تمّ");
    }
