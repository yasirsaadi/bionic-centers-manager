 ...
 * القاعدة عند كلّ طلب** ويعودان **مع الطابور** (`/api/medical-review/queue`)
 * — لا من الجلسة. واشتقاقُهما من `AiAccessContext` يكذب في الاتجاهين:
 * طبيبٌ سُحب اختصاصُه، أو مديرُ فرعٍ في نطاقه. ولذلك
 * `medicalReviewGuide()` **لا تأخذ `AiAccessContext` أصلاً** كأختها.
 */
 ...
 * وتعودان **في جسم الطابور نفسِه**. فاشتقاقُهما من الجلسة يكذب في
 * الاتجاهين: طبيبٌ سُحب اختصاصُه يبقى في جلسته «طبيباً»، ومديرُ فرعٍ يملك
 * الإشرافَ في نطاقه ولا يقول دورُه وحدَه ذلك. **فما لا تأخذه الدالّةُ لا
 * تستطيع أن تدّعيه.**
 *
 * **ولا خريطةَ تسمياتٍ ثانية**: القرارات من `REVIEW_DECISION_LABELS`
 * بترتيب `REVIEW_DECISIONS`، وأسبابُ الزيارة من `REVIEW_KIND_LABELS`،
 * والاختصاصان من `REVIEW_SERVICE_TYPES` + `specialtyLabel`.
 */
...
- **مرشِّحُ الاختصاص**: يظهر **فقط حين تُرجع النقطةُ أكثرَ من اختصاصٍ واحد**
  لهذا الحساب. **ولا تُشتقّ اختصاصاتُه من الجلسة** — تُقرأ من القاعدة وتعود
  مع الطابور.

**صفوفُ المراجعة**: **هي ما ترجعه \`GET /api/medical-review/queue\` وحدَه**
ضمن النافذة المختارة ونطاقِ الفرع واختصاصاتِ الحساب. **فلا تقل إن كلَّ طلبِ
مراجعةٍ لا بدّ أن يظهر هنا** — شروطُ الإرجاع يحسمها الخادم.
