...
- **وسؤالٌ عن قسمٍ بعينه** («كم إيراد العلاج الطبيعي؟»، «كم بعنا أطرافاً في آب؟») يُجاب من byDepartment **الجاهز** الذي تُعيده financial_summary مع current وكلّ صفٍّ في byBranch — لكلّ قسمٍ label عربيّ وsalesValue (ما بِيع) وrevenue (ما قُبض) بنفس تفريق الأداة تماماً. **ولا تجمع دلاءه بنفسك ولا تطرحها**: مجموع الخمسة يساوي الإجماليّ أصلاً، ومالُ الأجهزة كلِّه يصل جاهزاً في devicesCombined (تجميعٌ مشتقّ — **لا تجمعه مع دلاء byDepartment** وإلّا حسبتَ مالَ الأجهزة مرّتين). و«أجهزة قديمة — غير مقسَّمة» **مبيعاتٌ فقط**: revenue تصل null — قل «لا نظيرَ مقبوضاً مقاساً له» ولا تقرأها صفراً ولا توزّعها على قسمٍ بالتخمين، وكذلك «غير مبوَّب». **ولا مصاريفَ ولا صافيَ لقسمٍ بعينه**: المصاريف تصل إجماليةً للفترة لا مقسَّمةً هنا، فلا تقل إن قسماً رابحٌ أو خاسر ولا تطرح له مصروفاً.
 ...
 * طويلة تُغري النموذج بتعدادها للمستخدم.
 *
 * ══ ⚠ تصحيحٌ — العَلَمُ المخزَّن ليس مجملَ السلطة (٢٠٢٦-٠٩-١٩) ════════════
 * كانت الكتلةُ تسمّي هذه «صلاحياتِه الحقيقية» ثمّ تقول «**وما لم يُذكَر
 * أعلاه فهو غيرُ ممنوح**» — وهي **كذبةٌ على المسؤول العام**:
 ...
 * وعمودُ `can_view_reports` **افتراضُه `false` في المخطَّط**، فمسؤولٌ أُنشئ
 * بالافتراضات كان يُقال له «لا تملك `canViewReports`» وهو يملكها فعلاً.
 *
 * فمكانَ جملةِ النفي قاعدةٌ تمنع اختراعَ منعٍ من غياب عَلَم. **ولا يُملأ
 * العَلَمُ للمسؤول** (ذاك يزوّر مصدرَه)، **ولا يتغيّر حارسٌ ولا أداةٌ ولا
 * `AiAccessContext`** — النصُّ وحده.
 *
 ...
 * آخر. **والقاعدةُ الحرجة كما هي**: ليست مجملَ سلطته الفعلية، وغيابُ
 * عَلَمٍ لا يُثبت منعاً. **و`buildStoredPermissions` لم تُمَسّ.**
 */
function identityBlock(access: AiAccessContext): string {
  ...
  lines.push(granted.length > 0
    ? `- أعلامُ الصلاحيات المفعَّلة في الجلسة: ${granted.join("، ")}`
    : "- أعلامُ الصلاحيات المفعَّلة في الجلسة: لا شيء");
...
وهذه **أعلامُ صلاحياتٍ مفعَّلةٌ في جلسته — لا بالضرورة أعمدةٌ مخزَّنةٌ
مباشرةً على حسابه**: بعضُها يُقرأ من عمود الحساب، وبعضُها يُشتقّ من الدور
أو من عَلَمٍ آخر. **وهي على كلّ حالٍ ليست مجملَ سلطته الفعلية**.
فاستعملها لتشرح له **لماذا** قد تظهر له شاشةٌ أو إجراءٌ أو لا يظهر، بهذه
القاعدة:

- **غيابُ عَلَمٍ من القائمة لا يُثبت وحدَه أن الإجراءَ أو الشاشةَ ممنوعةٌ
  عليه.**
- فبعضُ حُرّاس التطبيق القائمة تمنح الوصولَ **أيضاً** عبر \`isAdmin\` أو عبر
  الدور، بمعزلٍ عن هذه القائمة.
- فإن شرحتَ إجراءً أو شاشةً بعينها فاحتكم إلى **حارسها القانونيّ** متى
  عرفتَه، لا إلى القائمة وحدها.
- **وإن لم تعرفه فلا تخترع منعاً من غياب عَلَم** — قل إنك لا تستطيع الجزم
  بلا حارسها.

ولا تسرد له ما لا يملك، قل ما ينقصه عند الحاجة فقط.
