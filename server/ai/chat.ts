 ...
 * الدور، بينما كلُّ حارسٍ في الخادم `isAdmin || permissions.X`. وعمودُ
 * `can_view_reports` **افتراضُه `false` في المخطَّط**، فمسؤولٌ أُنشئ
 * بالافتراضات كان يُقال له «لا تملك `canViewReports`» وهو يملكها فعلاً.
 *
 * فالوصفُ صار صادقاً — **أعلامٌ مخزَّنةٌ مفعَّلة**، لا مجملُ السلطة — ومكانَ
 * جملةِ النفي قاعدةٌ تمنع اختراعَ منعٍ من غياب عَلَم. **ولا يُملأ العَلَمُ
 * للمسؤول** (ذاك يزوّر المخزَّن)، **ولا يتغيّر حارسٌ ولا أداةٌ ولا
...
function identityBlock(access: AiAccessContext): string {
  ...
  lines.push(granted.length > 0
    ? `- أعلامُ الصلاحيات المخزَّنة المفعَّلة: ${granted.join("، ")}`
    : "- أعلامُ الصلاحيات المخزَّنة المفعَّلة: لا شيء");
...
وهذه **أعلامُ صلاحياتٍ مخزَّنةٌ على حسابه — لا مجملُ سلطته الفعلية**.
...
- فبعضُ حُرّاس التطبيق القائمة تمنح الوصولَ **أيضاً** عبر \`isAdmin\` أو عبر
  الدور، بمعزلٍ عن العَلَم المخزَّن.
