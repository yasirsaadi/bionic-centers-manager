...
//
// ══ **الغيابُ ≠ الفراغ** (تصحيحٌ — مراجعةٌ حيّة) ═══════════════════════════
// `branchId: ""` كان يُقرأ مثل `undefined`/`null` تماماً — سلسلةٌ فارغة أو
// بياضٌ محض **وصلت صراحةً** في الطلب، وليست غياباً للمفتاح. المفتاحُ
// الغائب حقّاً هو الغيابُ الصحيح الوحيد؛ فراغٌ مُرسَل عمداً أو بالخطأ من
// النموذج يُرفَض الآن كأيّ قيمةٍ أخرى لا تصلح رقم فرع.
async function resolveAdminRequestedBranchId(
  ...
  //  ══ فراغٌ صريح **حضورٌ لا غياب** — يُرفَض لا يُقرأ «كلّ الفروع» ══
  if (typeof raw === "string" && raw.trim() === "") {
    return { ok: false, error: "رقم الفرع (branchId) لا يمكن أن يكون فارغاً." };
  }
  const n = Number(raw);
  ...
  financial_summary: {
    spec: {
      ...
      description:
        ...
        + "وcurrent وكلُّ صفٍّ في byBranch يحملان byDepartment — تفصيلُ **الفترة نفسِها** بالأقسام: "
        + "prosthetic وmedical_support وphysiotherapy وlegacyDevicesUnsplit (أجهزةٌ قديمة غير "
        + "مقسَّمة) وunclassified (غير مبوَّب)، لكلٍّ label عربيّ وsalesValue (ما بِيع) وrevenue "
        + "(ما قُبض) بنفس التفريق أعلاه. **مجموعُ الخمسة يساوي الإجماليّ أصلاً فلا تجمعه ولا "
        + "تطرحه**، ومالُ الأجهزة كلِّه جاهزٌ في devicesCombined (تجميعٌ مشتقّ — **ليس دلواً "
        + "سادساً ولا يُجمع مع byDepartment**). وrevenue تصل null في legacyDevicesUnsplit: "
  ...
  device_sales_summary: {
    spec: {
      ...
      description:
        ...
        + "**ومرّر اسمَ الفرع وحده لا العبارةَ كاملة**: «مركز بغداد» أو «فرع بغداد» ⟵ "
        + "branchName=\"بغداد\" — «مركز» و«فرع» كلمتان عامّتان ليستا من الاسم. "
        ...
        + "\n\n**وbyCategory يُفصّل ما بِيع فعلاً** — صفٌّ لكلّ صنف بعنوانٍ عربيّ جاهز "
        + "(label) وعددٍ (sold)، بالأكثر مبيعاً أوّلاً. فمنه تُجاب أسئلةُ الأصناف كلُّها: "
        + "**الأطرافُ الكاملة مفصَّلةً بنوع البتر** (احادي · ثنائي · اطراف سليكونية تعويضية)، "
        + "**والسليكونيُّ مفصَّلٌ بجزئه** (اصبع · كف · اذن · انف · محجر عين) — فسؤالُ «كم "
        + "إصبعاً سليكونياً بِيع؟» جوابُه صفُّ «اطراف سليكونية تعويضية — اصبع». **وكلُّ "
        + "جزءٍ يُباع مفرداً** باسمه (القالب · السليكون · الركبة · التيوب · الأدابتر · القدم "
        + "· الغلاف الإسفنجي · غلاف القدم). **والمساندُ الطبية الكاملة** صنفٌ قائم. "
        + "وtotals يحمل ثلاثةَ مجاميعَ جاهزة (prostheticFullDevices · prostheticComponents "
        + "· medicalSupportDevices) — **انقلها كما هي ولا تجمع بنفسك**. "
        + "\n**وtotalSold يبقى الأطرافَ الكاملة وحدها** (= totals.prostheticFullDevices)، "
        + "فلا تقل «إجمالي المبيعات» عنه وقد بِيعت أجزاءٌ ومساند. "
        + "\n**والصيانةُ ليست بيعاً فلا تظهر في أيّ صنف** — إن سُئلت عن عدد الصيانات فقل "
        + "إن هذه الأداة لا تعدّها. **وصنفُ «نوعٌ غير مسجَّل» ليس صفراً ولا تخميناً**: "
        + "أجهزةٌ بِيعت فعلاً ولم يُسجَّل نوعُ بترها في معاينتها — اذكرها كما هي ولا "
        + "توزّعها على الأنواع.",
      input_schema: {
        type: "object",
        properties: {
