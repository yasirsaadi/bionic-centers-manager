...
export const DESCRIBED: Record<string, { d: string; q?: string[]; f?: true }> = {
  //  ══ التصنيع ══
  "/api/manufacturing/overview": { d: "لوحةُ أداء التصنيع: لكلّ خبيرٍ عددُ أوامره والنشطةُ والمكتملةُ و**المتأخّرةُ بدون عذرٍ مكتوب** (`overdue` — «المتأخرون» بتعريف المالك) و**المتأخّرةُ بعذر** (`overdueExcused`) كلٌّ على حدة، وعددُ إعادات العمل (`reworks`) ونسبةُ نجاح أوّل قياس ومتوسّطُ المدّة، ومعها تفصيلٌ بالفروع وبالمراحل وأسبابُ إعادة العمل.", q: ["branchId", "serviceType"] },
  "/api/manufacturing/notifications": { d: "أوامرُ التصنيع قربَ موعد التسليم أو بعده، صفّاً صفّاً: متأخّرة · اليوم · غداً · بعد يومين. ولكلّ صفٍّ المريضُ والخبيرُ والفرعُ والمرحلة، **وسببُ التوقّف إن وُجد** — وهو ما يقسم المتأخّرَ قسمين في الشاشة كلوحة التصنيع: «متأخرة بدون عذر» (الأحمر) و«متأخرة بعذر» (الكهرمانيّ). و`kind` واحدٌ للاثنين (`overdue`)، فالفارقُ `holdReasonCode` وحدَه.", q: ["branchId"] },
