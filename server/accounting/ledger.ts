...
/**
 * **إقصاءُ مال المريض المحذوف عن القوائم المالية الفعّالة** (مراجعة
 * ٢٠٢٦-٠٨-٢٤ — القسمان ج ود) — **بلا كسر توازن القيد المزدوج**.
 *
 * ══ لماذا على مستوى «القيد» لا على مستوى «السطر» ═══════════════════════
 * كلُّ كاتبٍ آليّ للقيود (`auto_journal.ts`: دفعة · فاتورة · دفعةُ فاتورة)
 * يكتب `patient_id` نفسَه على **كلّ** أسطر القيد الواحد — المدينُ والدائنُ
 * معاً. فحذفُ سطرٍ واحد فقط (مدينٍ بلا دائنه) كان سيُنتج ميزانَ مراجعةٍ
 * غيرَ متوازن — بالضبط ما تحذّر منه المراجعة. **فالإقصاءُ يقع على القيد
 * كاملاً**: إمّا كلُّ أسطره تُحسَب أو لا سطرَ منها يُحسَب، فيبقى مدينُ
 * الفترة = دائنُها دائماً.
 *
 * **وقيدٌ يدويّ (`POST /api/accounting/v2/journal-entries`) قد يخلط عدّة
 * مرضى في أسطره** — تلك حرّيةُ المحاسب. مثلُ هذا القيد **لا يُستثنى أبداً**
 * (`COUNT(DISTINCT patient_id) = 1` تفشل)، فلا يُخمَّن نصيبُ مريضٍ واحدٍ من
 * قيدٍ مختلط، ولا يختفي مالُ مريضٍ آخرَ فعّالٍ شارك القيدَ نفسَه.
 *
 * ══ وطرفان لا واحد ═══════════════════════════════════════════════════
 * ١) **حذفٌ ناعم** (قابلٌ للاستعادة) — يُفحَص **حيّاً** عند كلّ استعلام:
 *    القيدُ المُقصى هو ما كانت أسطرُه **كلُّها** لمريضٍ واحدٍ لا يزال صفُّه
 *    قائماً وعليه `deleted_at IS NOT NULL` الآن. فحين يُستعاد الملفُّ يعود
 *    القيدُ فوراً — لا حاجةَ لمسِّ صفٍّ واحد، لأن الفحص ديناميكيٌّ لا مخزَّن.
 *
 * ٢) **حذفٌ نهائيّ** (`storage.deletePatient`) — الكاسكيدُ ينزع
 *    `journal_lines.patient_id` (يجعله `NULL`) **قبل** أن يُمحى صفُّ
 *    المريض، فلا يبقى بعدها ما يُثبت أن هذا القيدَ كان مالَ ذلك المريض —
 *    والفحصُ الحيّ في (١) يعمى عنه فيعود المالُ المحذوفُ نهائياً إلى
 *    القوائم الفعّالة بصمت. **وهذا بالضبط ما لا يجوز.**
 *
 *    فالحلّ: `journal_entries.purged_patient_money` — **حقيقةٌ دائمة**
 *    تُكتب في **اللحظة نفسِها** التي تُنزَع فيها `patient_id` (قبلها
 *    مباشرةً، وهي تقرأ ما كانت عليه الأسطر قبل النزع)، لا حقلٌ يُحسَب لاحقاً
 *    أو يُصان بخلفيةٍ دورية. وهي **لا تُمسَح أبداً** — الحذفُ النهائيّ لا
 *    رجعةَ فيه، فالإقصاءُ الذي يوثّقه لا رجعةَ فيه كذلك.
 *
 * ══ وما لا يتغيّر ═══════════════════════════════════════════════════════
 * القيدُ نفسُه **لا يُحذف ولا يُعدَّل ولا يُعكَس** — يبقى بمبلغه وأسطره
 * كما كُتب، والتدقيقُ التاريخيّ يقرؤه كاملاً. الإقصاءُ قراءةٌ لا كتابة:
 * `getTrialBalance` وحدها (ومنها يُشتقّ قائمةُ الدخل والميزانيةُ العمومية)
 * تتوقّف عن عدّه ضمن الفعّال.
 */
const PATIENT_DELETION_EXCLUSION = `
  NOT (
    COALESCE(je.purged_patient_money, FALSE) = TRUE
    OR EXISTS (
      SELECT 1 FROM (
        SELECT jl2.entry_id, MAX(jl2.patient_id) AS pid
          FROM journal_lines jl2
         WHERE jl2.entry_id = jl.entry_id
         GROUP BY jl2.entry_id
        HAVING COUNT(DISTINCT jl2.patient_id) = 1
           AND COUNT(jl2.patient_id) = COUNT(*)
      ) ep
      JOIN patients dp ON dp.id = ep.pid
...
export async function getTrialBalance(filters: {
  ...
  const conditions: string[] = ["je.status = 'posted'", PATIENT_DELETION_EXCLUSION];
  const params: any[] = [];
  let paramIdx = 1;
