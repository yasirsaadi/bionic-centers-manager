...
// (٢) **حتميّة الرمز**: صفٌّ قديمٌ (معرّفُه n) ⟶ WB-nnnnn بحشو رقمه هو —
//     مُختبَرٌ بمعرّفٍ يملكه هذا الاختبارُ (لا بـ«الصفّ ١» تحديداً: قاعدةُ
//     الاختبار مشتركة، ومعرّفٌ منخفض قد يعود لصفٍّ آخر لاحقاً).
...
async function main() {
  ...
  same("ب. القديم الصغير أخذ رمز رقم صفّه",
    ...
    fmt(small.id));
  //  **لا فحصَ إضافياً على «الصفّ ١» بعينه بحثاً عن WB-00001** (حُذف
  //  ٢٠٢٦-٠٨-٢٤): كان يتّكئ صامتاً على أن يبقى الصفُّ الأصليُّ صاحبُ
  //  المعرِّف ١ في قاعدة الاختبار المشتركة بلا حذفٍ إلى الأبد — حالةٌ
  //  عرضيّة لا ثابتاً حقيقياً. ملفّاتُ اختبارٍ أخرى (مثل
  //  `manufacturing/isolation.test.ts`) تُنشئ صفوفاً بمعرّفاتٍ صريحة منخفضة
  //  خاصّتها، فمعرِّفٌ ١ قد يعود لصفٍّ **جديد** أُنشئ بعد هذا الترحيل —
  //  ورمزُه حينها من التسلسل الحيّ بحقّ، لا WB-00001، وهذا ليس عطباً.
  //  **والحتميّةُ الموثَّقة (٢) تبقى مُختبَرةً بالكامل** أعلاه عبر «ب» —
  //  بمعرّفٍ يملكه هذا الاختبارُ نفسُه ويتحكّم به، لا بمعرّفٍ عابر.
