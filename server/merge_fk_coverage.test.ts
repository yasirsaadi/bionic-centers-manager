...
const MERGE_DECISIONS: Record<string, string> = {
  ...
  //  سجلُّ التصحيح الإداريّ (ترحيل ٠٦٤): تاريخُ «صُحّحت هذه العملية» يتبع
  //  صاحبَه. ولا تصادمَ ممكن — تفرّدُه على `followup_id` لا على المريض.
  "administrative_operation_reversals.patient_id": "repoint — سجلّ التصحيح يتبع صاحبه",
  ...
  //  طلباتُ تصحيح الدفعات (ترحيل ٠٧١): اللقطةُ (`before_snapshot`) تحمل
  //  هويّةَ المريض لحظةَ الطلب، فطلبُ المصدر المعلَّق يُغلَق تلقائياً
  //  (رفضاً بلا تطبيق، بلا معتمِدٍ بشريّ) — بلا شرط تصادم، لأن التقادم لا
  //  الفهرس هو الخطر هنا. والتاريخُ كلُّه (المعلَّقُ المُغلَق والمُقرَّرُ
  //  سلفاً) ينتقل بمعرّفه.
  "financial_correction_requests.patient_id": "إغلاق المعلَّق تلقائياً (رفضٌ بلا تطبيق) ثم repoint — التاريخ كلُّه محفوظ",
