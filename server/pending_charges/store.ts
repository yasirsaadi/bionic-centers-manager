...
export async function createComponentSaleOperation(params: {
  ...
  return await db.transaction(async (tx) => {
    ...
    if (params.existingEpisodeId !== null) {
      ...
      //  ══ **حلقةٌ جديدة — عمليةٌ مستقلّة دائماً** (ترحيل ٠٧٣) ═══════════
      //  غيابُ `attachToDeviceEpisodeId` (لم يُسأل الموظّفُ، أو أجاب «لا»)
      //  يعني هذه هي: عمليةٌ جديدة تماماً. **`startDeviceEpisodeTx` تفتحها
      //  دائماً** — لم تعد ترفض لمجرّد أن الخيط يحمل حلقةً أخرى مفتوحة
      //  (قرارُ المالك: أيّ عددٍ من عمليات الأجهزة المستقلّة صحيحٌ لمريضٍ
      //  واحد). **وبلا تخمين لنيّة الموظّف كما كان**: «لا» أو صمتٌ لا
      //  يُلحِقان هذه العمليةَ بأيّ حلقةٍ قائمة أبداً — الإلحاقُ صريحٌ وحده
      //  عبر `attachToDeviceEpisodeId`.
      if (params.component === null) {
        throw new ChargeError("حدّد الجزء المراد بيعه — اختر من القائمة", 400);
      }
 ...
 *
 * **ولا حدَّ على عدد أوامر الصيانة المتزامنة** (ترحيل ٠٨٢، قرارُ المالك):
 * أيُّ عددٍ منها مفتوحٌ معاً — **حتى لنفس الجهاز ونفس القطعة ونفس الخبير**.
 * فهرسا ٠٥١ (`uq_pwo_one_open_maint_per_episode` /
 * `uq_pwo_one_open_legacy_maint`) رُفعا، وحارسُهما في
 * `createMaintenanceOrderWithVisit` حُذف معهما. **وكلُّ أمرٍ عمليةٌ مستقلّة
 * بمبلغها**: المبلغُ داخل معاملة أمرِه، فيقع معه أو يرتدّ معه — ولا يُقيَّد
 * مبلغُ أمرٍ مرّتين، ولا يُحسَب أمرٌ على أمرٍ آخر.
 *
