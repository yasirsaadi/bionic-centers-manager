...
export async function completeReceptionSale(params: {
  ...
  const body = async (tx: any): Promise<CommercialResult & { payment: Payment | null }> => {
    ...
    if (paidNow > 0) {
      ...
      //  **ولا دفعةً مبهمة**: بلا حالةٍ أو فرعٍ من الصفّ المباع، الهويّةُ
      //  ناقصة — فتسقط المعاملةُ كلُّها (البيعُ معها) بدل أن تُكتب دفعةٌ لا
      //  يُعرَف صاحبُها. `deviceEpisodeId` وحده يبقى مقبولاً `null` — رصيدُ
      //  جهازٍ غيرُ مخصَّص واقعةٌ صحيحة، لا التباساً.
      if (sold.caseId === null || sold.branchId === null) {
        throw new FollowupError(
          "تعذّر إسناد الدفعة إلى ملفّ المريض بثقة — لم تُسجَّل الدفعة."
          + " البيعُ تمّ؛ سجّل الدفعة من نافذة «تسجيل دفعة» المعتادة.",
          409,
        );
      }
