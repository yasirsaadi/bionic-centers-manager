...
async function main() {
  ...
  //  ── والمساعدُ الماضي يتخلّل بين الأداة و«فتح» (ارتدادُ #342) ───────────
  //  الفحصُ على المُلاصِق، وسابقُ «فتح» في «متى **تم** فتح أمر العمل؟» هو
  //  `تم` لا `متى` — فكانت تمرّ سؤالَ إجراء. فصارت «تم» منها.
  check(!isCurrentPageOrWorkflowQuestion("متى تم فتح أمر العمل؟"),
    "ط.٧د٢أ **«متى تم فتح أمر العمل؟» واقعةٌ ماضية** — «تم» تلاصق «فتح»");
  check(!isCurrentPageOrWorkflowQuestion("تم فتح أمر العمل؟"),
    "ط.٧د٢ب **و«تم فتح أمر العمل؟» كذلك** — بلا أداةِ سؤالٍ أصلاً");
  //  ── و«بعد ما تم فتح» + صيغةُ متابعةٍ مؤكَّدة (ارتدادُ #344) ────────────
  //  الإسقاطُ غيرُ المشروط كان يمحو **الإشارةَ الوحيدة** في سؤال متابعةٍ
  //  حقيقيّ، فيُحرَم من سياق صفحته. **والاستثناءُ شرطان مجتمعان** لا قاعدةٌ
  //  تُعمَّم: «بعد ما» مُلاصِقتين قبل «تم» وبترتيبهما، **ومعهما** إحدى
  //  صيغتَي المتابعة المؤكَّدتين.
  check(isCurrentPageOrWorkflowQuestion("بعد ما تم فتح أمر العمل شنو التالي؟"),
    "ط.٧د٢ج **«بعد ما تم فتح أمر العمل شنو التالي؟» سؤالُ متابعةٍ حقيقيّ**");
  check(isCurrentPageOrWorkflowQuestion("بعد ما تم فتح أمر العمل ما الخطوة التالية؟"),
    "ط.٧د٢د **وصيغةٌ أخرى منه كذلك** — «ما الخطوة التالية؟»");
  //  **والافتتاحُ وحدَه لا يكفي**: هذه تابعةُ «بعد ما» وسؤالُها مع ذلك عن
  //  واقعةٍ ماضيةٍ أخرى — فلا تصير سؤالَ إجراءٍ بافتتاحها.
  check(!isCurrentPageOrWorkflowQuestion("بعد ما تم فتح أمر العمل متى تم إغلاقه؟"),
    "ط.٧د٢د٢ **و«بعد ما تم فتح أمر العمل متى تم إغلاقه؟» واقعةٌ ماضية** — «بعد ما» وحدَها لا تكفي");
  //  **والصيغةُ تتابعٌ حرفيٌّ لا كلماتٌ متفرّقة**: «شنو» و«التالي» حاضرتان
  //  هنا ومتباعدتان («شنو صار في اليوم التالي») — فليست صيغةَ المتابعة.
  check(!isCurrentPageOrWorkflowQuestion("بعد ما تم فتح أمر العمل شنو صار في اليوم التالي؟"),
    "ط.٧د٢د٣ **و«…شنو صار في اليوم التالي؟» واقعةٌ ماضية** — كلماتٌ متفرّقة لا صيغةُ متابعة");
  //  **و«تم فتح» ماضٍ بالأصل مهما سبقه** — ولا يُقرأ تبعيّةً بغياب أداةِ
  //  سؤال. قاعدةٌ أوسع كانت تقرأ هذه الثلاثَ متابعةً وهي أسئلةُ واقعة.
  check(!isCurrentPageOrWorkflowQuestion("من تم فتح أمر العمل؟"),
    "ط.٧د٢هـ **و«من تم فتح أمر العمل؟» تبقى واقعةً ماضية**");
  check(!isCurrentPageOrWorkflowQuestion("هل تم فتح أمر العمل؟"),
    "ط.٧د٢و **و«هل تم فتح أمر العمل؟» كذلك** — سؤالُ وقوعٍ لا إجراء");
  check(!isCurrentPageOrWorkflowQuestion("اليوم تم فتح أمر العمل؟"),
    "ط.٧د٢ز **و«اليوم تم فتح أمر العمل؟» كذلك** — ظرفٌ لا افتتاحُ جملةٍ تابعة");
  //  **والفحصُ على المُلاصِق لا على حضور الأداة**: «متى **يتم** فتح صيانة؟»
  //  سؤالُ إجراءٍ حقيقيّ — أداةُ السؤال فيه لا تسبق «فتح» مباشرةً.
  //  **و«يتم» ليست «تم»** — وهي الفارقُ كلُّه: مضارعٌ يُبتلَع يُسقط سؤالَ
  //  الإجراء معه، فيعود العطبُ من الجهة الأخرى.
