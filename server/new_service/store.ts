  ...
  const body = async (tx: any): Promise<NewServiceResult> => {
    ...
    if (entries) {
      for (const entry of entries) {
        ...
        //  **والمجّانيُّ يُسجَّل ولا يُسقَط**: دفعةٌ بمبلغ صفر موسومةٌ
        //  `isFreeSessions` — فالجلسةُ تُعدّ وقيمتُها المالية صفر. وإسقاطُ
        //  الصفّ كان سيخفي الجلسةَ عن عدّاد مريض المفرد كلّه.
        if (entry.cost > 0 || isFree) {
