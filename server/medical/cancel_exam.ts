  ...
  return await db.transaction(async (tx) => {
    ...
    if (episodeId !== null) {
      ...
      if (ep) {
        ...
        //  «لم يشترِ» ثمّ «عاد للشراء» ثمّ معاينةٌ ثانية على الحلقة نفسِها:
        //  صار للجهاز معاينتان فعّالتان، والأحدثُ هي الحاكمة — بها تنتظر
        //  متابعتُه الحيّةُ قرارَ المريض. وكان إلغاءُ **الأولى** يُرجع الحلقةَ
        //  إلى «بانتظار المعاينة» بينما الثانيةُ حيّةٌ والاستعلاماتُ تعرض
        //  «إتمام البيع» عليها: الطبيبُ يراه منتظراً فحصاً، والبائعُ يراه
        //  مفحوصاً، والبيعُ يُردّ ٤٠٩ «قبل معاينة الطبيب» — تناقضٌ ولّده
        //  إلغاءُ ما لم يعد سلطةً أصلاً.
        //
        //  وكانت متابعةُ «لم يشترِ» **المنتهية** تُعاد كتابتُها «أُغلقت
        //  بسبب إلغاء المعاينة»: ماضٍ لم يقع، ويُخفي قرارَ المريض الحقيقيّ.
        //
        //  فالقاعدة: تُقاس **أحدثُ** معاينةٍ فعّالةٍ على الحلقة بعينها
        //  (بترتيب `signed_at` ثمّ `id` — نفسُ ترتيب قرّاء «معاينة الحلقة»).
        //  إن وُجدت أحدثُ من هذه ⟶ **٤٠٩ وصفرُ كتابة**: لا شاهدةَ، ولا
        //  إرجاعَ حلقة، ولا مسَّ متابعة، ولا سطرَ تدقيق. والرسالةُ **تسمّي
        //  الأحدثَ بعينها** فيعرف الطبيبُ ما الذي يحكم الجهازَ الآن.
        //  فالمعايناتُ تُسحَب على الجهاز **من الأحدث إلى الأقدم**: مَن أراد
        //  سحبَ توقيعه عن قديمةٍ خاطئة يُلغي ما بعدها أوّلاً، فتبقى
        //  السلسلةُ متّسقةً في كلّ لحظة ولا يقع رفضٌ ضمنيّ صامت.
        //  وإن كانت هذه هي الأحدثَ ⟶ تُرجَع الحلقةُ كما كان دائماً:
        //  **الطلبُ باقٍ، والمعاينةُ سقطت**، وتُوقَّع مصحَّحةٌ تطالب الحلقةَ
        //  نفسها ولا تفتح ثانيةً.
        const newer = await tx.execute<{ id: number }>(sql`
          SELECT me2.id
            FROM medical_exams me2
            JOIN medical_exams me ON me.id = ${params.examId}
           WHERE me2.device_episode_id = ${episodeId}
             AND me2.id <> me.id
             AND (me2.signed_at > me.signed_at
                  OR (me2.signed_at = me.signed_at AND me2.id > me.id))
             AND ${activeExamSql("me2")}
           ORDER BY me2.signed_at DESC, me2.id DESC
           LIMIT 1
        `);
        const newerRow = (newer.rows ?? [])[0];
        if (newerRow) {
          throw new ExamCancelError(
            `لا يمكن إلغاء هذه المعاينة: للجهاز نفسِه معاينةٌ أحدثُ فعّالة`
            + ` (#${Number(newerRow.id)}) هي الحاكمة الآن.`
            + ` ألغِ الأحدثَ أولاً إن كانت هي الخطأ —`
            + ` أو راجع المدير المسؤول إن كانت لطبيبٍ آخر.`,
            409,
          );
        }
    ...
    if (fu) {
      ...
      // ══ **المتابعةُ المنتهية لا يُعاد كتابةُ تاريخها** (INT-01) ═════════
      //  «لم يشترِ» واقعةٌ قالها المريضُ يومَها وسُجّل سببُها بحرفه؛ إلغاءُ
      //  المعاينة لاحقاً لا يجعله «أُغلقت بسبب إلغاء المعاينة» — ذاك يكتب
      //  ماضياً لم يقع، ويُخفي «لم يشترِ» عن «تم الحسم» والمراجعة اليومية.
      //  فيتقاعد بالإلغاء ما كان **حيّاً** وحده.
      if (!isTerminal(fst)) followupRetired = Number(fu.id);
    }
