...
// storage.ts: الحلقة كيانٌ له ثوابت دقيقة (تسلسلٌ لا يتكرّر، وحالاتٌ خمس
// بانتقالات محدودة)، وتوزيعُ حراستها على عدّة ملفات هو كيف تنحرف الثوابت
// بصمت.
//
// **و«شراءٌ مفتوحٌ واحد لكلّ خيط» لم يعد منها** (ترحيل ٠٧٣ — قرارُ المالك):
// أيُّ عددٍ من عمليات الأجهزة المستقلّة لمريضٍ واحد في آنٍ واحد صحيح.
//
// ══ ما تملكه الحلقة ═════════════════════════════════════════════════════
...
/**
 * الحالات التي تعني «الشراء ما زال جارياً».
 *
 * كانت مطابقةً لشرط `uq_pde_case_open`، **والفهرسُ رُفع في ترحيل ٠٧٣** فلم
 * يعد خلفها فهرسٌ يحصر المفتوحَ في واحد. وتبقى القائمةُ نفسُها لأنها تجيب
 * سؤالاً آخر ما زال قائماً: **أيُّ الحلقات حيّةٌ الآن** — تقرؤها القوائمُ
 * والطوابيرُ وحُرّاسُ أمر العمل، لا حارسُ تفرّدٍ ملغى.
 */
 ...
 *
 * ══ وحلقةٌ مفتوحةٌ غيرُ مرتبطةٍ **لا تُختطَف ولا تَمنع** (قرارُ المالك
 *    ٢٠٢٦-٠٩-١٥ — يُلغي ويستبدل الردَّ ٤٠٩ الذي كان هنا) ══════════════════
 * هذه الدالّة لا تُنادى إلّا حين `followup.device_episode_id IS NULL` —
 * فالمريضُ العائد ذو الطلب الصريح لا يمرّ بها أصلاً.
 *
 ...
 * حالةً تاريخيةً غيرَ متّسقة. فالتقاطُها **اختطافٌ** يُثبّت سعرَ هذه
 * المتابعة على جهازٍ لم يُقصَد ويُنسَب أمرُ تصنيعه إليه — **وهذا ما زال
 * ممنوعاً بحرفه: لا تُقرأ حلقةٌ قائمة ولا تُلتقَط ولا تُعدَّل ولا تُغلَق.**
 *
 * **والذي تغيّر أن الالتباسَ لم يعد يُردّ بل يُحَلّ**: يُفتَح لهذه المتابعة
 * **جهازٌ جديدٌ مستقلٌّ خاصٌّ بها**، ويمضي البيعُ على مساره المعتاد. فترحيلُ
 * ٠٧٣ (أيُّ عددٍ من عمليات الأجهزة المستقلّة لمريضٍ واحد في آنٍ واحد) جعل
 * **تعدّدَ الحلقات المفتوحة هو الحالَ الطبيعيّ لا الشذوذ** — فالردُّ ٤٠٩
 * صار يحبس بيعاً مشروعاً لمجرّد أن للمريض جهازاً آخر يُصنَع، ولا يد للموظّف
 * فيه ولا بابَ يفكّه.
 *
 * **والعزلُ هو الضمانة لا الردّ**: الجديدةُ حلقةٌ منفصلة بتسلسلها الخاصّ،
 * فسعرُ هذه المتابعة وأمرُ تصنيعها وقيدُ كلفتها تشير إليها وحدها — ولا صفٌّ
 * قائمٌ يُمَسّ بحرف. **ولا قيدَ في القاعدة تغيّر**: `uq_pde_case_seq` يحرس
 * التسلسل كما كان، و`uq_pde_case_open` مرفوعٌ منذ ٠٧٣ فلا شيءَ يمنع الثانية.
 *
 ...
 * قبل الوصول إلى هنا (الثانيةُ تقرأ `converted` فتُردّ)، ويحرس التسلسلَ
 * `uq_pde_case_seq` في القاعدة. **و`uq_pde_case_open` لم يعد منها** —
 * رُفع في ترحيل ٠٧٣، فالحصرُ هنا يأتي من قفل المتابعة وقفلِ الخيط لا من
 * فهرسِ تفرّدٍ على «المفتوح».
 *
 * **ولا تُستعمَل «حلقةٌ مفتوحةٌ موجودة» وسيلةَ عدم-تكرارٍ لمتابعةٍ مجهولة
 * الهويّة** — تلك تجعل الالتباسَ يمرّ صامتاً بدل أن يُردّ.
 *
 * @returns الحلقةُ المُنشأة **دائماً** حين يوجد خيطٌ لهذا الاختصاص — سواءٌ
 *          أكان للمريض أجهزةٌ أخرى مفتوحة أم لا — أو `null` حين لا خيطَ
...
export async function ensureFirstDeviceEpisodeForSale(
  ...
  params: {
    ...
    requestedItem?: RequestedItem | null;
    /**
     * **فرعُ القرار الذي يُباع** (قرارُ المالك ٢٠٢٦-٠٩-٢٥) — الجهازُ يُفتَح
     * حيث يقع البيع، فأمرُه وكلفتُه هناك. والغيابُ فرعُ الحالة كما كان.
     */
    branchId?: number | null;
  },
): Promise<LockedEpisode | null> {
  ...
  //
  //  كان هنا فحصٌ يقرأ أيَّ حلقةٍ ليست `delivered`/`cancelled` على الخيط
  //  فيردّ ٤٠٩. وقد وُضع يومَ كان تعدّدُ الحلقات المفتوحة شذوذاً يستحقّ
  //  إنساناً يحسمه؛ ثمّ رفع ترحيلُ ٠٧٣ `uq_pde_case_open` فصار التعدّدُ
  //  **هو الحالَ المشروع**. فبقاؤه كان يحبس بيعاً صحيحاً لمجرّد أن للمريض
  //  جهازاً آخر يُصنَع — بلا بابٍ يفكّه.
  //
  //  **والحلُّ ليس التقاطَ القائمة بل تجاهُلَها**: تُفتَح لهذه المتابعة
  //  حلقتُها هي، فلا تُقرأ حلقةٌ قائمة ولا تُقفَل ولا تُعدَّل ولا تُغلَق.
  //  والاختطافُ الذي وُضع الحارسُ لمنعه **ممنوعٌ كما كان** — بالامتناع عن
  //  الالتقاط لا بالردّ.
  //
  //  **والقفلُ الباقي كافٍ**: قفلُ صفّ الخيط أعلاه هو نقطةُ التسلسل نفسُها
  //  التي يستعملها `startDeviceEpisodeTx`، فـ`MAX + 1` أدناه يُحسَب تحته
  //  ويحرسه `uq_pde_case_seq`. وقفلُ المتابعة في `confirmPurchase` يمنع
  //  ضغطتين من إنتاج حلقتين لنفس المتابعة (الثانيةُ تقرأ `converted`).
 ...
 * تسلسلٌ لا يتكرّر، وتصنيفُ جزءٍ يلازم نوعَ الخدمة، وأمرُ بناءٍ قديمٍ بلا
 * هويّة يمنع فتحَ بديلٍ فوقه — وتوزيعُ حراستها على منسّقاتٍ متعدّدة هو
 * **كيف تنحرف الثوابت بصمت**. فالتصحيحُ الإداريّ يستعمل الحرّاسَ القائمة
 * ولا ينسخها.
 *
 * **و«شراءٌ مفتوحٌ واحد لكلّ خيط» لم يعد منها** (ترحيل ٠٧٣): أيُّ عددٍ من
 * عمليات الأجهزة المستقلّة على الخيط الواحد صحيحٌ الآن — راجع ④ أدناه.
 *
 ...
 * لا لأن حارساً يمنع غيرَ ذلك — ذاك الحارسُ رُفع مع `uq_pde_case_open`
 * (④ أدناه) — بل لأن المعنى نفسَه يقتضيه: التصحيحُ يُبطل الخطأ **ثمّ**
 * يفتح الصحيح، فنداءٌ قبل الإبطال يترك الخاطئةَ مفتوحةً بجانب بديلها
 * ويُنتج طلبين لعمليةٍ واحدة. والترتيبُ في المنسّق كما هو.
 *
 * ══ وما لا تفعله ════════════════════════════════════════════════════════
...
export async function createReplacementEpisodeTx(
  ...
  //  ══ ④ **ولا يُردّ البديلُ لمجرّد وجود حلقةٍ أخرى مفتوحة** (ترحيل ٠٧٣) ══
  //     كان هنا حارسٌ يقرأ أيَّ حلقةٍ ليست `delivered`/`cancelled` على الخيط
  //     فيردّ ٤٠٩ — نصُّ شرطِ `uq_pde_case_open` الذي **رفعه ترحيلُ ٠٧٣**
  //     (قرارُ المالك: أيُّ عددٍ من عمليات الأجهزة المستقلّة لمريضٍ واحد في
  //     آنٍ واحد صحيح). و`startDeviceEpisodeTx` أسقطه يومَها؛ **وهذه الدالّةُ
  //     نُسيت** — فصارت الشقيقتان تحكمان الخيطَ نفسَه بقاعدتين مختلفتين.
  //
  //     وأثرُه أن **التصحيحَ الإداريَّ كلَّه يسقط** حين يحمل الخيطُ عمليةً
  //     ثانيةً مستقلّةً حيّة: المنسّقُ يُبطل الخاطئة ويعكس مالَها ثمّ ينادي
  //     هذه، فترتدّ ٤٠٩ فتُرجع المعاملةَ بكاملها (بحكم `tx` المُجبَرة أعلاه).
  //     فالمريضُ يبقى محبوساً في خطئه **لأن له عمليةً أخرى سليمة** — وتلك
  //     العمليةُ لا شأنَ لها بالتصحيح ولا يجوز أن تُمَسّ ولا أن تَمنع.
  //
  //     **والحمايةُ الحقيقية الباقية أضيق ولم تُمَسّ**: أمرَا عملٍ يتنافسان
  //     على **حلقة الجهاز نفسها** يمنعهما `uq_pwo_one_open_build_per_episode`
  //     ومثيلاتُها **عند إنشاء أمر العمل**، لا هنا — وفتحُ حلقةٍ لا يفتح
  //     أمراً بنفسه. وهو الشرحُ نفسُه المكتوب في `startDeviceEpisodeTx`. ══
