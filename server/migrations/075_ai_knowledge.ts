...
VALUES
  ('patient_journey_overview', 'مسار المريض في النظام',
   'مسار المريض: يبدأ بالتسجيل في الاستقبال (الاسم، الهاتف، الفرع، ونوع الإصابة إن كان طرفاً صناعياً أو مسنداً طبياً أو علاجاً طبيعياً). بعدها يحتاج المريض معاينة طبيب في اختصاصه (أطراف صناعية / مساند طبية / علاج طبيعي) قبل أي تخصيصٍ أو بيع لجهاز — إلا في حالات استثنائية محدودة يقرّرها النظام تلقائياً (مريض قديم مثلاً). بعد المعاينة يُنجز موظف الاستعلامات خطوة "إتمام البيع" (السعر والخصم والخبير) فيبدأ التصنيع، أو يُسجَّل "لم يشترِ" بسبب. العلاج الطبيعي يمرّ بمسار منفصل بلا حاجة لجهاز.',
  ...
  ('reception_new_patient_registration', 'تسجيل مريض جديد',
   'يُسجَّل المريض من شاشة "مريض جديد": الاسم والهاتف والفرع إلزاميون، ومربّع موافقة واتساب مرفوعٌ افتراضياً (يُرسل رسالة ترحيب تلقائية على الرقم المسجَّل). الحقول السريرية التفصيلية لحالات الأطراف الصناعية والمساند الطبية (مثل نوع البتر وجهته) تظهر فقط لمدير الفرع والطبيب والمسؤول العام — الاستقبال لا يُدخلها لأن الطبيب يحدّدها لاحقاً في المعاينة. أما إصابات المريض وتاريخها وتصنيفه فتبقى من عمل الاستقبال كالمعتاد. حفظ الملف يُنشئ رمز مريض علنيّاً ثابتاً بصيغة WB-xxxxx لا يتغيّر أبداً.',
  ...
  ('manufacturing_stages', 'مراحل تصنيع الطرف الصناعي',
   'بعد إتمام بيع الجهاز يبدأ أمر التصنيع بمراحله: استلام الأمر، ثم القياسات، ثم القالب، ثم التصنيع، ثم "جاهز للتجربة"، وأخيراً التسليم. الأمر يُسنَد إلى خبير واحد يتابعه حتى النهاية. تغيير موعد التسليم لأمر قائم يتطلّب كتابة سبب صريح يظهر لاحقاً في بطاقة المريض. لا يبدأ التصنيع أبداً إلا بعد معاينة طبيب موقّعة لنفس الاختصاص، إلا في مسارات "بلا معاينة" المحدودة (جلسات إضافية، صيانة، بيع جزء) التي يفتحها الاستقبال مباشرة والطبيب يراجعها لاحقاً استرجاعياً بلا أثر على قراره.',
  ...
  ('financial_correction_request', 'طلب تصحيح مالي على دفعة',
   'تعديل أو حذف بيانات مالية حسّاسة لدفعة (المبلغ، تاريخها، نوع العلاج المدفوع، أو حذفها بالكامل) لا يقع مباشرة — يُنشئ الموظف طلب تصحيح يحمل السبب ولقطة كاملة من الحالة قبل التعديل، وينتظر قرار مسؤول مخوَّل. لا يُطبَّق أي تغيير مالي فعلي حتى الاعتماد الصريح. الملاحظات وعدد الجلسات وحدها تبقى قابلة للتعديل الفوري بلا طلب لأنها لا تمسّ المال مباشرة.',
   'finance', 'النظام (ترحيل ٠٧٥)', 'اعتمادٌ ذاتيّ عند الترحيل — يراجعه المسؤول العام عند الحاجة'),

  ('administrative_reversal', 'التصحيح الإداري لعملية خاطئة',
   'عندما تقع عملية بيع أو تصنيع بالخطأ، يستطيع المسؤول العام (بلا قيد فرع) أو مدير الفرع (ضمن فرعه) فتح "التصحيح الإداري" من ملف المريض. هناك خياران: "تراجع عن الشراء فقط" إن كان الطلب والمعاينة صحيحين والخطأ في ضغطة الشراء وحدها، أو "إلغاء العملية بالكامل" إن كانت العملية كلها خاطئة. كلا الخيارين يعكس الكلفة المالية تلقائياً ويحتفظ بالسجل التاريخي كاملاً — لا شيء يُحذف. لا يعمل هذا التصحيح بعد تسليم الجهاز فعلياً.',
