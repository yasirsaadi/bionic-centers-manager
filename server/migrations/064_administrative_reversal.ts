 ...
 *
 * ══ ④ ولا تصحيحان **لصفقةٍ واحدة** ══════════════════════════════════════
 * التفرّدُ على `followup_id` مدى الحياة كان يقفل البابَ إلى الأبد: «تراجع عن
 * الشراء» يُعيد **المتابعةَ نفسَها** إلى ما قبل البيع، فتُشترى بعدها بشكلٍ
 * صحيح ويولد **أمرُ تصنيعٍ ثانٍ** — ولو احتاج ذاك تصحيحاً لرُدّ بحجّة تصحيحٍ
 * قديمٍ لا علاقةَ له به. وذلك نقضُ الغرض: المسؤولُ يصحّح **العمليةَ القائمة**.
 *
 * فالهويّةُ الحقيقية للتصحيح المالي **الصفقةُ بعينها**: أمرُ التصنيع، فتفرّدٌ
 * جزئيٌّ عليه. وطلبٌ لم يُبَع قطّ لا أمرَ له، فإبطالُه الكامل حدثٌ طرفيٌّ
 * يقع مرّةً — وله تفرّدُه الجزئيّ الخاص. والضغطةُ المزدوجة يحسمها كلاهما.
 *
...
-- ══ ولا تصحيحان **لصفقةٍ واحدة** — لا لمتابعةٍ مدى الحياة ═══════════════
--  التفرّدُ على المتابعة وحدها كان يقفل البابَ إلى الأبد: «تراجع عن الشراء»
--  يُعيد المتابعةَ نفسَها إلى ما قبل البيع، فتُشترى بعدها بشكلٍ صحيح ويولد
--  **أمرُ تصنيعٍ ثانٍ**. ولو احتاج ذاك الأمرُ تصحيحاً يوماً لرُدّ بحجّة
--  «صُحّحت هذه المتابعة سابقاً» — **وهو نقضٌ للغرض كلِّه**: المسؤولُ يجب
--  أن يصحّح **العمليةَ القائمة** دائماً.
--
--  فالهويّةُ الحقيقية للتصحيح المالي هي **الصفقةُ بعينها**: أمرُ التصنيع.
--
--  والفهرسُ القديم يُسقَط صراحةً أوّلاً: «IF NOT EXISTS» لا تُرقّي موجوداً.
DROP INDEX IF EXISTS uq_aor_followup;

CREATE UNIQUE INDEX IF NOT EXISTS uq_aor_work_order_once
  ON administrative_operation_reversals (work_order_id)
  WHERE work_order_id IS NOT NULL;

--  وطلبٌ لم يُبَع قطّ لا أمرَ له، وإبطالُه الكامل حدثٌ طرفيّ يقع مرّةً.
--  (و«تراجع عن الشراء» بلا أمرٍ لا معنى له أصلاً، فيُردّ في الشيفرة.)
CREATE UNIQUE INDEX IF NOT EXISTS uq_aor_unsold_followup_void
  ON administrative_operation_reversals (followup_id)
  WHERE work_order_id IS NULL AND mode = 'full_operation';
