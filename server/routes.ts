...
export async function registerRoutes(
  ...
  app.delete("/api/patients/:id/case-type/:caseType", isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      //  ══ **ولا نصَّ Postgres خامّاً على أيّ مسار** ═══════════════════════
      //  القرارُ يُؤخَذ تحت قفل المريض، لكنّ فتحَ طلبِ جهازٍ متزامناً لا يأخذ
      //  ذلك القفل — فقد يُدرَج صفٌّ جديد بين القرار والحذف فيرفع ٢٣٥٠٣.
      //  والمعاملةُ تتراجع كاملةً (لا نصفَ كتابة)، لكنّ الرسالةَ كانت تصل
      //  المستخدمَ بنصّ المفتاح الأجنبيّ. فتُترجَم إلى ما يفعله.
      if (String(err?.code) === "23503") {
        return res.status(409).json({
          message: "تغيّرت حالة الملف أثناء التنفيذ — أُضيف طلبٌ أو سجلٌّ جديد لهذا النوع.",
          code: "changed_during_write",
          remedy: "حدّث الصفحة وأعد المحاولة.",
        });
      }
  ...
  app.put(api.patients.update.path, isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      for (const [field, caseType, label] of [
        ...
        delete patch[field];
        //  **ويُسقَط من `req.body` أيضاً** — لا من `patch` وحده: حارسُ
        //  «التصحيحُ الإداريّ المحض» أدناه يقيس **الوارد** لا المصفّى، فحقلٌ
        //  أُسقط هنا ولم يُسقَط هناك كان يجعل تصحيحَ هاتفٍ يُقاس على أنه
        //  تغييرُ تصنيف، فيُردّ ٤٠٠ «بيانات ناقصة» بدل الملاحظة.
  ...
  app.post("/api/patients/:id/new-service", isAuthenticated, async (req, res) => {
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        //  **(تصحيحٌ لاحق)**: نصُّ الرسالة كان يقول «فيُعتمَد» — بقيّةٌ من
        //  قبل تقاعد اعتماد الخصم الحيّ (٢٦٠). لا اعتمادَ بعد اليوم، والخصمُ
        //  يُطبَّق فوراً عند الحفظ — والرسالةُ صارت تقوله صراحةً.
        if (isPhysioService && !wantsFree && !wantsCut && serviceCost !== stdPrice) {
          throw new NewServiceError(
            ...
            + ` وأيّ مبلغ أقلّ يُسجَّل من «خصم أو خدمة مجّانية» ويُطبَّق مباشرةً عند الحفظ —`
        ...
        if (isPhysioService && (wantsFree || wantsCut)) {
          ...
          const out = await discountStore.applyDiscountImmediatelyTx(tx, {
            ...
            payload: {
              ...
              //  قبضاً كاملاً. **ويُحتَرَم في مسار البنود أيضاً** (تصحيحٌ
              //  تشغيليّ — صدقُ الإيصال): يُوزَّع تناسبياً على البنود بدل
              //  أن تُقيَّد حصّةُ كلّ بندٍ من الكلفة دفعةً كاملة.
