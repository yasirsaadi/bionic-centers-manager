...
//     المراجعة السريعة (`pending` + `quick`) **وهو وحدَه بأثرٍ رجعيّ**،
//     وقسمُ `awaitingFull` طلباتٌ لم تقع معاينتُها بعد. وما يستوجب المسارَ
//     الكامل (`requiresFullPath`) لا يُعرَض سبباً لبطاقةٍ سريعة.
...
async function main() {
  ...
  check(/\*\*⚠ وهذه وحدَها\s+بأثرٍ رجعيّ\*\*/.test(mr)
    && /فليست موافقةً سابقةً\s+لتنفيذها/.test(mr),
    "ح.٢ب **وهي وحدَها الرجعيّة، وليست إذناً سابقاً**");
  check(/لا تحجب عملاً ولا ديناراً ولا أمرَ\s+تصنيع/.test(mr),
    "ح.٢ج **ولا تحجب عملاً ولا مالاً**");
  check(/\*\*فلا تصفه بأنه بأثرٍ رجعيّ\*\*/.test(mr)
    && /ما زالت\s+تنتظر معاينةً طبيةً كاملة\*\* لم تقع بعد/.test(mr),
    "ح.٢د **وقسمُ «بانتظار الطبيب» ليس رجعياً**");
  ...
  check(/\*\*⚠ ولا تُشتقّ هذه القائمةُ من\s+الجلسة ولا من دور المستخدم\*\*/.test(mr),
    "ح.٤أ **ولا تُشتقّ من الجلسة ولا من الدور**");
