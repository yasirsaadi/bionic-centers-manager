...
// (٧) **و«معايناتي» دليلٌ ساكن**: مرشِّحُ الفرع فيه من **صفوف القائمة** لا من
...
async function main() {
  ...
  check(/المرضى المنتظرون معاينةَ \*\*هذا الطبيب\*\*/.test(my), "ز.٢ **الغرضُ: مَن ينتظر معاينتَه هو**");
  ...
  check(/ليس نقصَ صلاحية/.test(my), "ز.٦ب وغيابُه ليس نقصَ صلاحية");

  //  ③ **مرشِّحُ الفرع من الصفوف لا من النطاق** — الفرقُ الجوهريّ عن ٤أ.
  check(/تحمل صفوفُ القائمة المعروضة الآن أكثرَ من\s+فرعٍ واحد/.test(my),
    "ز.٧ **مرشِّحُ الفرع: من صفوف القائمة**", my.slice(0, 1400));
  check(/مشتقٌّ من الصفوف لا من نطاق عمل المستخدم/.test(my),
