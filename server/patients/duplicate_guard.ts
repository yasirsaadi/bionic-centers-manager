...
//  (`pg_advisory_xact_lock`) على هاش النصّ المطبَّع، يُقفَل **قبل** الفحص
//  وطَوال المعاملة، فمحاولتان متزامنتان بنفس المفتاح تتسلسلان — الثانية
//  تنتظر ثمّ تقرأ ما كتبَته الأولى فتُرفَض بصدق. والمفتاحان (اسمٌ/هاتف)
//  بمساحتَي أسماءٍ منفصلتين كي لا يتصادما، ومختلفتان عن `919` (قفلُ صفّ
//  المريض القائم في storage.ts) لأنّ لا صفَّ مريضٍ موجوداً بعد عند التسجيل.
//
//  **ولا حمايةَ لسباقٍ بين اسمين مختلفين بادئةً واحدُهما للآخر** (مفتاحا
//  قفلٍ مختلفان لا يتسلسلان) — نطاقٌ ضيّقٌ مقصود: المطلوبُ حمايةُ نفس الاسم
//  المطبَّع من التسجيل المزدوج المتزامن، لا كلّ تشكيلات البادئة النظرية.
...
/**
 * **بابُ الإنفاذ الوحيد على التسجيل**: يقفل مفتاح الاسم المطبَّع استشارياً
 * (طوالَ المعاملة الجارية) ثمّ يفحص — فلا سباقَ بين قفلٍ وقراءة. يُستدعى
 * **داخل** معاملة `storage.createPatient` قبل أيّ `INSERT`، لا قبلها ولا
 * بعدها. **لا يُستعمَل على `PUT` أبداً** — القاعدةُ للتسجيل وحده.
 */
export async function assertNameAvailableForRegistration(
  ...
  await tx.execute(
    sql`SELECT pg_advisory_xact_lock(${PATIENT_NAME_LOCK_NAMESPACE}, hashtext(patient_search_norm(${name})))`,
  );
