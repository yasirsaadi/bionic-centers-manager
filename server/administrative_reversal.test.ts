...
async function main() {
  ...
  try {
    ...
    //  **تسلسلٌ لا يتكرّر** (`uq_pde_case_seq`). والتصحيحُ الإداريّ يفتح
    //  حلقةً كأيّ فاتحٍ آخر — فيخضع لها لا يستثنى منها.
    //
    //  **و«شراءٌ مفتوحٌ واحد لكلّ خيط» لم يعد منها** (ترحيل ٠٧٣ رفع
    //  `uq_pde_case_open`): أيُّ عددٍ من عمليات الأجهزة المستقلّة لمريضٍ
    //  واحد في آنٍ واحد صحيح. فالمُختبَرُ هنا **الذرّيةُ والتسلسل**، لا
    //  حصرُ المفتوح في واحد.
    // ══════════════════════════════════════════════════════════════════
    console.log("\n── م) التزامن والذرّية ──");
    {
      ...
      // (ب) **طلبُ جهازٍ جديد يسابق الاستبدال** ⟶ **كلاهما يمضي**.
      //     المسارُ العاديُّ والتصحيحُ يقفلان صفَّ الخيط نفسَه فيتسلسلان،
      //     ويخرج المريضُ بعمليتين مستقلّتين مفتوحتين — البديلُ عن العملية
      //     المصحَّحة، والطلبُ الجديد المستقلّ عنها. **وهذا صحيحٌ منذ ترحيل
      //     ٠٧٣**: كان يُردّ أحدُهما بحكم `uq_pde_case_open`، والفهرسُ رُفع.
      //     والمُختبَرُ الباقي أن **الخاطئةَ وحدَها تُلغى** وأن **التسلسلَ
      //     لا يتكرّر** مهما كان ترتيبُ السباق.
      ...
      check(newReq.status < 400,
        "١٣٩. **والطلبُ الجديد يمضي أيضاً** — لا يُردّ لوجود عمليةٍ أخرى",
        `${newReq.status} / ${JSON.stringify(newReq.body)}`);
      const open2 = s2.eps.filter((e: any) =>
        !["delivered", "cancelled"].includes(String(e.status)));
      same("١٤٠. **فعمليتان مستقلّتان مفتوحتان** — البديلُ والطلبُ الجديد",
        open2.length, 2);
      //  والخاطئةُ وحدَها أُلغيت — لا العملياتُ الحيّة.
      same("١٤٠-ب. **والملغاةُ واحدةٌ: الخاطئةُ وحدَها**",
        s2.eps.filter((e: any) => String(e.status) === "cancelled").length, 1);
      //  والتسلسلُ يبقى فريداً مهما كان ترتيبُ السباق.
      const seqs2 = s2.eps.map((e: any) => Number(e.sequence_number));
      same("١٤٠-ج. **ولا تسلسلَ مكرّر تحت السباق**",
        seqs2.length, new Set(seqs2).size);
