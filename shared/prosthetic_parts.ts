...
//  الحارسان قبل القاعدة التي تستعملهما: `noExamSaleServiceTypes` تُحسَب
//  **لحظةَ تحميل الوحدة**، فلو بقيا تحتها لسقط الاستيرادُ كلُّه في منطقة
//  الموت الزمنيّ (TDZ) — والثابتُ المشتقّ يستحقّ ترتيباً يصحّ لا تعليقاً.
...
/**
 * **الجهازُ الكاملُ لا يُباع بلا معاينة — طرفاً كان أو مسنداً.**
 *
 * الجزءُ بديلٌ لقطعةٍ وُصفت يوماً على جهازٍ قائم — قالبٌ يبلى أو ركبةٌ
 * تنكسر — فبيعُه قرارٌ تشغيليّ. أمّا **الجهازُ الكاملُ فقرارٌ سريريٌّ من
 * أوّله**: مستوى البتر ونوعُ المفصل والقدمُ والمقاس للطرف، والقياسُ
 * والجهةُ والمستوى للمسند. كلُّها يقرّرها طبيب.
 *
 * ══ والقاعدةُ صُحِّحت بقرار المالك (بعد ٢٤٩) ═══════════════════════════
 * كان الاستثناءُ يقول: «المساندُ بلا قائمة أجزاء، فالمسندُ الكاملُ يبقى
 * مؤهَّلاً». وهذا **قلبٌ للحجّة**: غيابُ قائمة الأجزاء يعني أن المسندَ
 * **لا يُباع بلا معاينة إطلاقاً** — لا أنه يُباع كاملاً بلا معاينة. فلم
 * يكن للمساند مسارُ بيعٍ بلا معاينة صحيحٌ قطّ، وكان البابُ المفتوح يبيع
 * **أشدَّ** ما يحتاج الطبيب: الجهازَ كلَّه.
 *
 * فالمسموحُ بلا معاينة هو **جزءُ طرفٍ صناعيّ وحده**. وما عداه يُردّ
 * **ولا يُصحَّح بصمت**: تحويلُ المسار تلقائياً يدّعي قراراً لم يتّخذه أحد.
 *
 * **وصيانةُ المسند لا تُمَسّ**: جهازٌ قائمٌ يُصلَح — لا جهازَ يُوصَف.
 */
const NO_EXAM_SALE_REFUSAL_TAIL =
  "افتح طلباً على مسار المعاينة، أو صحّح هذا الطلب من «تصحيح / إلغاء العملية».";

/**
 * **قاعدةُ البيع بلا معاينة — واحدةٌ يقرؤها الخادمُ والشاشةُ معاً.**
 *
 * تُرجع `null` حين يجوز البيع، ورسالةَ الردّ الصادقة حين لا يجوز. ورسالةٌ
 * واحدة مشتقّةٌ من نوع الخدمة أصدقُ من ثابتٍ يسمّي الطرفَ في وجه مسند.
 *
 * @param item ما طُلب. والغيابُ (`null` لصفٍّ قديم) يُقرأ «جهازاً كاملاً»
 *   كما في `requestedItemLabel` — فيُردّ، وهو الصواب.
 */
export function noExamSaleRefusal(serviceType: unknown, item: unknown): string | null {
  //  **الجزءُ وحده**، وللأطراف وحدها. والأجزاءُ لا توجد لغيرها أصلاً
  //  (`parseRequestedItem` تردّها)، فالشرطان يقولان الشيء نفسه من وجهين.
  if (serviceType === "prosthetic" && isProstheticComponent(item)) return null;
  const label = isDeviceServiceKind(serviceType)
    ? FULL_DEVICE_LABELS[serviceType] : FULL_DEVICE_NEUTRAL_LABEL;
  return `${label} يحتاج معاينة الطبيب — لا يُباع من «بلا معاينة». `
    + NO_EXAM_SALE_REFUSAL_TAIL;
}

/** ويجوز البيعُ بلا معاينة؟ — نفسُ القاعدة، بلا نسخةٍ ثانية منها. */
export const noExamSaleAllowed = (serviceType: unknown, item: unknown): boolean =>
  noExamSaleRefusal(serviceType, item) === null;

/**
 * **وقسمٌ كامل بلا بيعٍ بلا معاينة إطلاقاً** — تقرؤه الشاشةُ قبل أن تعرض
 * خياراً يردّه الخادمُ بعده. والمساندُ اليومَ هي هذا القسم: لا جزءَ لها
 * يُباع، فلا يبقى إلّا الكاملُ — وهو ممنوع.
 */
export const noExamSaleServiceTypes: DeviceServiceKind[] =
  DEVICE_SERVICE_TYPES.filter((s) =>
    REQUESTED_ITEMS.some((i) => noExamSaleAllowed(s, i)));
