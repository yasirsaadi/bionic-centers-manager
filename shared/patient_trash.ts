 ...
 *
 * **بادئةٌ لا مفتاحٌ كامل** — استعمِل `trashBadgeSeenKey` أدناه دائماً،
 * لا هذا الثابت مباشرةً (تصحيحٌ لاحق، 2026-08-31: كان مفتاحاً واحداً
 * يتشاركه كلُّ مَن يدخل من نفس المتصفّح، فموظّفةٌ تفتح السلّة تُطفئ الشارةَ
 * لزميلها الذي يدخل بعدها من نفس الجهاز ولم يرَ شيئاً).
 */
export const TRASH_BADGE_LAST_SEEN_KEY = "patient-trash-badge-last-seen-at";

/**
 * مفتاحُ الشارة **لكلّ مستخدمٍ لا لكلّ متصفّح** — Sidebar.tsx وPatientTrash.tsx
 * كلاهما يبنيان المفتاحَ من هنا حصراً، فلا ينحرف أحدُهما عن الآخر.
 *
 * **`userId` غائبٌ في حالةٍ حقيقية واحدة لا عابرة**: مفتاحُ المسؤول القديم
 * («legacy admin key») لا يحمل حساب مستخدمٍ حقيقياً فيبقى بلا `userId` طوال
 * الجلسة لا لحظة تحميلٍ فقط. فيأخذ الغيابُ سلّةً ثابتة مشتركة `"shared"` —
 * نفسُ السلوك القديم (مفتاحٌ واحد) لذلك الدخول تحديداً وحده، لا لأيّ حساب
 * موظّفٍ حقيقيّ آخر يحمل `userId`.
 */
export function trashBadgeSeenKey(userId: number | null | undefined): string {
  return `${TRASH_BADGE_LAST_SEEN_KEY}:${userId ?? "shared"}`;
}
...
/**
 * **بوّابةُ الحذف النهائي** — لا تُفتَح قبل انقضاء مهلة الاستعادة.
 *
 * ثلاثون يوماً **ضمانةٌ** لا مجرّدَ عدّاد: مريضٌ حُذف بالخطأ يستحقّها
 * كاملةً قبل أن يصير قرارُه لا-رجعةَ فيه. ولو استطاع المسؤولُ العام
 * تجاوزَها لصار «حذف نهائي» بابَ حذفٍ فوريّ بزيّ سلّة — والمسؤولُ العام
 * **لا يُستثنى من هذا الشرط**، خلافاً لاشتراط الالتزام الماليّ الذي
 * يُستثنى منه (تلك سلطةٌ ماليّة، وهذه مهلةٌ زمنيّة للجميع بلا استثناء).
 */
export const PURGE_BEFORE_EXPIRY_MESSAGE = "لا يمكن الحذف النهائي قبل انتهاء مدة الاستعادة";
