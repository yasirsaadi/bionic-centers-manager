...
/**
 * **رمزُ رفضِ إلغاء المعاينة حين وقعت العمليةُ فعلاً** — بيعٌ اعتُمد، أو أمرُ
 * تصنيعٍ فُتح، أو جهازٌ صار `in_manufacturing`/`delivered`.
 *
 * الرفضُ نفسُه لم يتغيّر بحرف: إلغاءُ المعاينة **لا يتراجع عن مالٍ ولا عن
 * تصنيع** (`server/medical/cancel_exam.ts`)، ويبقى ٤٠٩ برسالته. الجديدُ أن
 * الردَّ صار يحمل **رمزاً آلياً** بجانب الرسالة، فتعرف الشاشةُ أن هذا الرفضَ
 * بعينه له بابٌ آخر — «تصحيح / إلغاء العملية» — بدل أن تُقرأ الرسالةُ نصّاً
 * وتنتهي عندها.
 *
 * **ولا يُقرأ الرمزُ إذناً**: مَن يفتح ذلك البابَ تقرّره `mayReverse` في
 * `server/admin_reversal/routes.ts` وحدها، والنقطتان تفحصان بأنفسهما. فالرمزُ
 * يقول «هذا الرفضُ له مسارٌ» لا «أنت تملكه».
 *
