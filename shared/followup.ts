 ...
 *
 * ══ والقراءةُ أوسعُ من الكتابة عمداً ════════════════════════════════════
 * الطبيبُ لا يتابع المريض تجارياً بعد اليوم، لكنه **يحتاج أن يرى**: أشترى
 * مريضُه أم لا، وبكم، وأين وقف الملفّ. فحجبُ القراءة عنه كان سيجعله يسأل
 * الاستعلامات في كل مرّة عن نتيجة قرارٍ هو مَن بدأه.
 *
...
/**
 * مَن **يعمل** في المتابعة التجارية — الاستقبالُ ومديرُ الفرع والمسؤول.
 *
 * ══ الفصلُ الذي تحرسه هذه الدالّة ═══════════════════════════════════════
 * **الطبيبُ يملك القرار السريري، والفرعُ يملك القرار التجاري.** وكانت
 * القراءةُ والكتابة بوّابةً واحدة، فكان الطبيبُ يستطيع أن يؤجّل مريضاً
 * ويغلق ملفَّه «لم يشترِ» ويختار الخبير ويؤكّد الشراء فيبدأ التصنيع —
 * وكلُّها أفعالٌ تجارية وتشغيلية لا شأنَ للطبّ بها.
 *
 * وليس تضييقاً على الطبيب بل **رفعُ عبءٍ عنه**: قرارُ «لم يشترِ» يحتاج مَن
 * كلّم المريضَ هاتفياً وسمع سببَه، لا مَن رآه في العيادة قبل أسبوعين.
 *
 * ══ وما بقي للطبيب ═════════════════════════════════════════════════════
 * القراءةُ الكاملة · إشارةُ «يرغب بالشراء الآن» · وحسمُ طلبِ سعرٍ **قديم**
 * معلَّق (توافقٌ رجعي — انظر `canDecideLegacyPriceRequest`).
 *
 * **والنطاقُ الجغرافي ليس هنا**: يفرضه `canReachBranch` في كل نقطة من صفّ
 * المتابعة نفسه لا من الطلب.
 */
export function canActCommercially(s: FollowupSessionLike | null | undefined): boolean {
  if (s?.isAdmin === true) return true;
  //  **بالدور وحده**: طبيبٌ يحمل `canWriteMedicalExam` لا يصير تجارياً بها،
  //  ومديرُ فرعٍ يحملها لا يفقد تجاريّته — الدورُ هو ما يقرّر.
 ...
 * تحرسه البوّابة، والقديمُ كان يقول «متابعة» فبدا أنه يشمل الطبيبَ لأنه
 * يتابع مريضَه سريرياً — وهو ليس كذلك.
 */
 ...
 * ولا سلطةَ تُستأذَن لتسجيل ما وقع — ولذلك لا اعتمادَ هنا لأحد.
 *
 * ══ ولماذا خرج الطبيبُ منها ════════════════════════════════════════════
 * لأن هذه الضغطة **تبدأ تصنيعاً وتقيّد كلفةً على حساب المريض**: فعلٌ
 * تشغيليٌّ ماليّ بحت. وكان الطبيبُ يملكها لأن البوّابة كانت واحدةً لكلّ
 * مَن يلمس الملفّ — لا لأن أحداً قرّر أن يبيع الطبيبُ. وبقاؤها بيده كان
 * يعني أن ضغطةً سهوية في غرفة الفحص تفتح أمرَ تصنيعٍ وتحرّك مالاً.
 *
 ...
 *
 * **والطبيبُ خارجها**: إسنادُ الخبير قرارٌ تشغيليّ — مَن يركّب الجهاز ومتى
 * يفرغ له — يعرفه الفرعُ لا العيادة. وكان الطبيبُ يملكه لأن البوّابة كانت
 * واحدةً لكلّ مَن يلمس الملفّ.
 */
export function canSelectExpert(
  ...
  return canActCommercially(s) && !isTerminal(status);
}
...
export function allowedActions(
  ...
  //  **والطبيبُ يخرج من هذه القائمة كلِّها إلّا إشارتَه** — لا لأن الشاشة
  //  تخفيها بل لأن `canActCommercially` و`canConfirmPurchase` تردّانه،
  //  والخادمُ يفحص الدالّتين نفسَهما على كل كتابة.
  const out: string[] = [];
